...
const MyFiles = () => {
  ...
  const [uploadForm, setUploadForm] = useState({
    file: null, sensitivity_level: 'low', project_category: '', department: ''
  });
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <div style={{ fontSize: '13px', color: '#64748B', marginTop: '3px' }}>
            {currentFolder ? (
              <span>
                <span style={{ cursor: 'pointer', color: '#22C55E', fontWeight: '500' }}
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {someSelected && canDelete && (
            <button className="neu-btn" onClick={handleBulkDelete} disabled={deleting}
      ...
      {showNewFolder && (
        <div className="neu-raised" style={{ padding: '20px', marginBottom: '16px' }}>
          ...
          <form onSubmit={handleCreateFolder} style={{ display: 'flex', gap: '10px' }}>
            <input className="neu-input" type="text" placeholder="Folder name…"
        ...
        <div className="neu-raised" style={{ padding: '20px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
            {['file', 'folder'].map(mode => (
              <button key={mode} type="button"
                className={uploadMode === mode ? 'neu-btn-primary' : 'neu-btn'}
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
            <div className="neu-pressed" style={{
              flex: '2', minWidth: '180px', padding: '11px 16px',
            ...
            <select className="neu-input" style={fieldStyle}
            ...
            <input className="neu-input" style={fieldStyle} type="text" placeholder="Project tag"
            ...
            {uploadMode === 'file' && (
              <button className="neu-btn-primary" type="button" disabled={uploading}
                onClick={handleUpload} style={{ whiteSpace: 'nowrap', padding: '11px 20px' }}>
      ...
      {loading ? (
        ...
        <div style={{
          padding: '60px', textAlign: 'center', background: '#FFFFFF',
        ...
        <div>
          ...
          {visibleFolders.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '12px' }}>
                Folders
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(156px, 1fr))', gap: '12px' }}>
                {visibleFolders.map(folder => (
                  <div key={folder.id} className="neu-raised"
                    ...
                    <svg width="30" height="30" viewBox="0 0 24 24" fill="#DBEAFE" stroke="#3B82F6" strokeWidth="1.5"
                    ...
                    {canDelete && (
                      <button className="neu-btn"
                        ...
                        style={{ position: 'absolute', top: '8px', right: '8px', padding: '2px 7px', fontSize: '13px', color: '#DC2626', border: 'none', background: 'transparent' }}>
                        ×
                      </button>
                    )}
                  </div>
              ...
              <div className="neu-raised" style={{ padding: '0', overflow: 'hidden' }}>
                ...
                <div style={{
                  ...
                  {canDelete && (
                    <div
                      style={{
                        ...
                        background: allSelected ? '#22C55E' : 'transparent',
                        flexShrink: 0,
                      }}
                  ...
                  {['File name','Sensitivity','Department','Project','Size','Actions'].map(h => (
                    <div key={h} style={{ fontSize: '11px', fontWeight: '600', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                      {h}
                    </div>
                  ))}
                </div>
                  ...
                  {visibleFiles.map((file, idx) => {
                    ...
                    return (
                      <div key={file.id} style={{
                        ...
                        borderBottom: idx < visibleFiles.length - 1 ? '1px solid #F8FAFC' : 'none',
                        ...
                        {canDelete && (
                          <div
                            style={{
                              ...
                              opacity: canDeleteThis ? 1 : 0.3, flexShrink: 0,
                            }}
                        ...
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <button className="neu-btn" style={{ padding: '5px 10px', fontSize: '11px' }}
                            onClick={() => handleDownload(file.id, file.original_name)}>
                            Download
                          </button>
                          {canDeleteThis && (
                            <button className="neu-btn" style={{ padding: '5px 10px', fontSize: '11px', color: '#DC2626' }}
                ...
                {someSelected && (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button className="neu-btn" style={{ fontSize: '12px', padding: '7px 14px' }}
                        onClick={() => setSelectedFiles([])}>
                        Clear
                      </button>
                      <button className="neu-btn" style={{ fontSize: '12px', padding: '7px 14px', color: '#DC2626' }}
