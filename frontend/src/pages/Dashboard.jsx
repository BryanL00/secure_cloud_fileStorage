...
const LINE_DATA = [42,60,55,80,70,95,130,115,145,90,60,40];

const LineChart = () => {
  const W = 580, H = 180, PAD = { top: 10, bottom: 30, left: 40, right: 10 };
  const maxV = Math.max(...LINE_DATA);
  const pts = LINE_DATA.map((v, i) => {
    const x = PAD.left + (i / (LINE_DATA.length - 1)) * (W - PAD.left - PAD.right);
    const y = PAD.top + (1 - v / maxV) * (H - PAD.top - PAD.bottom);
    return [x, y];
  });
  const d = pts.map(([x,y], i) => `${i===0?'M':'L'}${x.toFixed(1)},${y.toFixed(1)}`).join(' ');
  const fill = `${d} L${pts[pts.length-1][0].toFixed(1)},${(H-PAD.bottom).toFixed(1)} L${PAD.left},${(H-PAD.bottom).toFixed(1)} Z`;
  const yTicks = [0, 50, 100, 150, 200];

  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto' }}>
      {yTicks.map(v => {
        const y = PAD.top + (1 - v / 200) * (H - PAD.top - PAD.bottom);
        return (
          <g key={v}>
...
const ROLE_DATA = [
  { year: '2023', admin: 60, editor: 50, viewer: 30, contributor: 10 },
  { year: '2024', admin: 55, editor: 65, viewer: 45, contributor: 20 },
  { year: '2025', admin: 40, editor: 35, viewer: 55, contributor: 15 },
];
const ROLE_COLORS = { admin: '#22C55E', editor: '#4ADE80', viewer: '#BBF7D0', contributor: '#DCFCE7' };

const BarChart = () => {
  const W = 260, H = 180, PAD = { top: 10, bottom: 30, left: 10, right: 10 };
  const barW = 40, barGap = 40;
  const maxTotal = Math.max(...ROLE_DATA.map(d => d.admin + d.editor + d.viewer + d.contributor));

  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto' }}>
      {ROLE_DATA.map((d, i) => {
        const x = PAD.left + i * (barW + barGap) + 10;
        const chartH = H - PAD.top - PAD.bottom;
        const segments = [
          { key: 'contributor', v: d.contributor },
          { key: 'viewer',      v: d.viewer },
          { key: 'editor',      v: d.editor },
          { key: 'admin',       v: d.admin },
        ];
        let yOff = 0;
        return (
          <g key={d.year}>
            {segments.map(({ key, v }) => {
              ...
              const y = PAD.top + chartH - yOff - segH;
              yOff += segH;
              return <rect key={key} x={x} y={y} width={barW} height={segH} fill={ROLE_COLORS[key]} rx="2"/>;
            })}
            <text x={x + barW / 2} y={H - 6} textAnchor="middle" fill="#94A3B8" fontSize="9">{d.year}</text>
          </g>
        );
      })}
    </svg>
...
const CATEGORY_META = {
  documents: { label: 'Documents', icon: <IconDoc />, iconBg: '#EEF2FF', trend: '+24%', up: true },
  images:    { label: 'Images',    icon: <IconImg />, iconBg: '#FFFBEB', trend: '+12%', up: true },
  videos:    { label: 'Videos',    icon: <IconVid />, iconBg: '#FEF2F2', trend: '+8%',  up: true },
  others:    { label: 'Others',    icon: <IconOther />, iconBg: '#F8FAFC', trend: '+24%', up: true },
};
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (user?.role === 'Administrator') { await api.get('/users'); return; }
        const [filesRes] = await Promise.all([api.get('/files')]);
  ...
  const recentFiles = [...files].sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0)).slice(0, 5);
  ...
  return (
    <div>
          ...
          {Object.entries(CATEGORY_META).map(([key, meta]) => (
            <div key={key} className="neu-raised" style={{ padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '14px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: meta.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {meta.icon}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                  <Trend pct={meta.trend} up={meta.up} />
                  <span style={{ fontSize: '10px', color: '#94A3B8' }}>⋯</span>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginBottom: '4px' }}>
                <span style={{ fontSize: '26px', fontWeight: '700', color: '#0F172A', letterSpacing: '-0.5px' }}>
                  {counts[key]}
                </span>
                <span style={{ fontSize: '13px', color: '#64748B', fontWeight: '500' }}>files</span>
              </div>
              <div style={{ fontSize: '11px', color: '#64748B' }}>
                {meta.label} · {fmt(sizes[key])}
              </div>
              <div style={{ fontSize: '10px', color: '#94A3B8', marginTop: '6px' }}>Jan 01, 2025 – Dec 30, 2025</div>
            </div>
          ))}
        </div>
      )}
      ...
      {user?.role === 'Administrator' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
          {[
            { label: 'Total Files', value: '—', sub: 'No file access' },
            { label: 'Storage Used', value: '—', sub: 'Admin restricted' },
            { label: 'Your Role', value: 'Admin', sub: user?.email },
            { label: 'Actions', btn: true },
          ].map((s, i) => (
            <div key={i} className="neu-raised" style={{ padding: '20px' }}>
              <div style={{ fontSize: '11px', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '10px' }}>{s.label}</div>
              {s.btn ? (
                <button className="neu-btn-primary" style={{ fontSize: '12px', padding: '8px 14px' }} onClick={() => navigate('/admin')}>
                  Manage Users
                </button>
              ) : (
                <>
                  <div style={{ fontSize: '24px', fontWeight: '700', color: '#0F172A', marginBottom: '4px' }}>{s.value}</div>
        ...
        <div className="neu-raised" style={{ padding: '20px', marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            <div style={{ fontSize: '14px', fontWeight: '600', color: '#0F172A' }}>
              Flow Forge{' '}
              <span style={{ color: '#64748B', fontWeight: '400' }}>Space&apos;s</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
            {[
              ...
              { dot: '#94A3B8', label: 'Unused space (12 GB)' },
            ].map(({ dot, label }) => (
          ...
          <div style={{ marginTop: '12px', fontSize: '12px', color: '#64748B' }}>
            When your team runs out of space, accounts will stop syncing and you won&apos;t be able to add any new files. To add more space,{' '}
            <span style={{ color: '#22C55E', cursor: 'pointer', fontWeight: '500' }}>contact support</span>.
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', marginBottom: '24px' }}>
          ...
          <div className="neu-raised" style={{ padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <div style={{ fontSize: '14px', fontWeight: '600', color: '#0F172A' }}>User Upload Activity</div>
          ...
          <div className="neu-raised" style={{ padding: '20px', minWidth: '260px' }}>
            <div style={{ fontSize: '14px', fontWeight: '600', color: '#0F172A', marginBottom: '16px' }}>
              Storage Consumption by User Role
            </div>
            <BarChart />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '10px' }}>
              {Object.entries(ROLE_COLORS).map(([role, color]) => (
                <div key={role} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: '#64748B', textTransform: 'capitalize' }}>{role}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="neu-raised" style={{ padding: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <button style={{
              ...
              color: '#475569', fontSize: '12px', fontWeight: '500', cursor: 'pointer',
            }}>
          ...
          {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Name', 'Uploader', 'Date Uploaded', 'Size'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '8px 12px', fontSize: '11px', fontWeight: '600', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid #F1F5F9' }}>
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  <tr key={file.id} style={{ borderBottom: '1px solid #F8FAFC' }}>
                    <td style={{ padding: '14px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '13px', fontWeight: '500', color: '#0F172A', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {file.original_name}
                          </div>
                          <div style={{ fontSize: '11px', color: '#94A3B8', textTransform: 'capitalize' }}>
                            {getFileCategory(file.original_name)}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '14px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{
                          width: '26px', height: '26px', borderRadius: '50%', background: '#22C55E',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          color: 'white', fontSize: '10px', fontWeight: '700', flexShrink: 0,
                        }}>
                          {(file.owner_email || 'U')[0].toUpperCase()}
                        </div>
                        <div>
                          <div style={{ fontSize: '12px', fontWeight: '500', color: '#0F172A' }}>
                            {file.owner_email?.split('@')[0] || '—'}
                          </div>
                          <div style={{ fontSize: '11px', color: '#94A3B8' }}>{file.owner_email || '—'}</div>
                        </div>
                      </div>
                    </td>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div className="neu-raised" style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              <button className="neu-btn-primary" onClick={() => navigate('/admin')} style={{ fontSize: '13px' }}>
                Manage Users
              </button>
              <button className="neu-btn" onClick={() => navigate('/admin')} style={{ fontSize: '13px' }}>
                View Audit Logs
              </button>
            </div>
          </div>
          <div className="neu-raised" style={{ padding: '24px' }}>
            <div style={{ fontSize: '14px', fontWeight: '600', color: '#0F172A', marginBottom: '10px' }}>File Access</div>
            <div className="neu-pressed" style={{ padding: '14px' }}>
